...
export function toggleErrorFor(feature, enabled) {
  if (errorFlags.hasOwnProperty(feature)) {
    ...
    console.log(`🧪 Error injection for ${feature} ${enabled ? 'enabled' : 'disabled'}`);
    updateUI();
  }
}
...
function createTestingUI() {
  ...
  Object.keys(errorFlags).forEach(feature => {
    ...
    label.appendChild(document.createTextNode(` ${formatFeatureName(feature)}`));
...
function updateUI() {
  ...
  Object.keys(errorFlags).forEach(feature => {
    const checkbox = container.querySelector(`input[data-feature="${feature}"]`);
    if (checkbox) {
      checkbox.checked = errorFlags[feature];
    }
  });
